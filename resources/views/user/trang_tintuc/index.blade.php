...
<div class="flex flex-col gap-16">
    <section class="relative overflow-hidden rounded-3xl bg-gradient-to-br from-primary/20 via-accent/15 to-primary/30 px-6 py-20 shadow-inner sm:px-10 lg:px-16">
        ...
        <div class="mx-auto flex max-w-5xl flex-col gap-8 text-center lg:text-left">
            ...
            <div class="space-y-5">
                <h1 class="text-3xl font-bold leading-tight text-base-content sm:text-4xl">Tin tức du lịch mới nhất</h1>
                <p class="text-base text-base-content/70">Cập nhật xu hướng điểm đến, kinh nghiệm hành trình và ưu đãi hấp dẫn để bạn luôn dẫn đầu xu hướng dịch chuyển.</p>
            </div>
            <div class="flex flex-wrap items-center justify-center gap-3 text-sm text-base-content/70 lg:justify-start">
                <span class="inline-flex items-center gap-2 rounded-full bg-base-100/80 px-4 py-2 shadow">{{ $trangTinTucs->count() }} bài viết đang hiển thị</span>
                <span class="inline-flex items-center gap-2 rounded-full bg-base-100/80 px-4 py-2 shadow">{{ $theLoais->count() }} chủ đề du lịch</span>
                <a href="#loc-tin-tuc" class="inline-flex items-center gap-2 text-primary font-semibold">Khám phá bộ lọc<span aria-hidden="true">&rarr;</span></a>
            </div>
        </div>
    </section>
        ...
        <div class="grid gap-8 lg:grid-cols-[320px_1fr]">
            <x-ui.card class="space-y-6 border border-base-200/80 bg-base-100/90 shadow-lg">
                <form id="category-filter-form" class="space-y-5">
                    <div class="space-y-2">
                        <h2 class="text-lg font-semibold text-base-content">Lọc theo thể loại</h2>
                        <p class="text-sm text-base-content/70">Chọn chủ đề bạn quan tâm để xem bài viết phù hợp.</p>
                    </div>
                    ...
                    <div class="flex flex-wrap gap-3">
                        <x-ui.button type="button" id="apply-filter" variant="primary" class="flex-1 justify-center">Áp dụng</x-ui.button>
                        <x-ui.button type="button" id="reset-filter" variant="ghost" class="justify-center">Bỏ chọn</x-ui.button>
                    </div>
                </form>
            </x-ui.card>
            ...
            <div class="space-y-8">
                <div class="space-y-2 text-center lg:text-left">
                    <h2 class="text-2xl font-semibold text-base-content">Tin tức nổi bật</h2>
                    <p class="text-sm text-base-content/70">Cập nhật những câu chuyện được quan tâm nhiều nhất trong tuần.</p>
                </div>
                    ...
                    @forelse($trangTinTucs as $trangTinTuc)
                        <x-ui.card hover class="flex h-full flex-col overflow-hidden p-0">
                            ...
                            <div class="flex flex-1 flex-col gap-4 px-6 pb-6 pt-5">
                                ...
                                <div class="mt-auto">
                                    <x-ui.button href="{{ route('showTinTuc', $trangTinTuc->slug) }}" variant="ghost" class="px-0 text-primary">Đọc tiếp</x-ui.button>
                                </div>
                            </div>
                        </x-ui.card>
                    @empty
                        <div class="lg:col-span-2">
                            <x-ui.empty title="Chưa có bài viết phù hợp" description="Hãy thử điều chỉnh bộ lọc hoặc quay lại sau để xem thêm bài viết mới." />
                        </div>
                    @endforelse
                </div>
